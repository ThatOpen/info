...
### Install libraries
  ...
  👩‍🏫
The first thing to do is to create an empty folder and start a new npm project with the command `npm init`. This will create a `package.json` file containing some data such as the project name, version, commands and dependencies. In addition, the following dependencies must be installed with npm:
...
- A **canvas element** that will be used to render the Three.js scene.
- An **input element**, which will open IFC files from our computer to the application.
- A **script** referencing a file called `Bundle.js`, which is the bundle of the app that will be created by rollup.
...
- `npm run watch` will activate the `watch mode`, updating that file automatically every time we make changes to the code and save.
...
This file is necessary because it contains the compiled C++ logic of <a href="https://github.com/IFCjs/web-ifc">web-ifc</a>, which is the parsing core to read and write IFC files with native speed
  ...
  <IfcAlert>
This files has to be served statically in your application. This might need different tweaks if you are using frameworks or libraries like React, Angular, Vue or Svelte.
  </IfcAlert>
...
```
  ...
  🎣
Keep in mind that if you haven't saved the file `web-ifc.wasm` in the root of the project, you'll need to specify it's location with `setWasmPath`. For instance, if we had store `web-ifc.wasm` in a folder called `wasm` contained in a folder called `static` in the root of the project, we would do the following:
...
This is just the beginning. You can take a look at web-ifc-viewer to <a href="https://github.com/IFCjs/web-ifc-viewer">web-ifc-viewer</a> to see the IFC viewer we are building, which includes tools for object selection, changing geometry appearance, section planes and much more. You can try it <a href="https://ifcjs.github.io/web-ifc-viewer/examples/simple-html/">here</a>.
  </IfcCard>
