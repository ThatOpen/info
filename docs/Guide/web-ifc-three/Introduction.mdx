...
With web-ifc you could build almost any BIM tool capable of reading and writing IFC. So why create a layer on top of it?
  ...
  🤷‍♀️
Because almost all BIM tools are 3D, and we think it would not make sense for all developers to implement their own viewer from scratch. If you are only interested in the data, you may be better off using web-ifc directly.
