...
<IfcCard>
  ☝
Using this method not mandatory, but if you want to be able to select objects in medium / large IFC models at 60 fps, it's necessary. You can install it with <i>npm i three-mesh-bvh</i> or <i>yarn add three-mesh-bvh</i>.
</IfcCard>
...
```
  ...
  💳
Gets the Express ID of an IFC element from a face index. Why a face index? Because when we select an object in 3d space with the mouse we get the index of that face. We usually want the ID of the object where that face belongs to in order to highlight it, isolate it or get all its properties.
...
```
🎸🪕🎻
Gets the IFC type of the specified element (e.g. IFCWALLSTANDARDCASE).
