...
we have seen how to obtain the ID of an object on which we cast a ray with the
<a href="https://threejs.org/docs/#api/en/core/Raycaster">Raycaster</a>. Now that we have those IDs, it's possible to highlight those elements using <b>geometric subsets</b>.
...
<IfcAlert>You can have two separate subsets with the same appearance using two instances of the same material.</IfcAlert>
...
<IfcAlert>This would allow, for example, to create a geometric subset with all the ground floor elements of the BIM model and hide the rest.</IfcAlert>
