...
<IfcAlert>Remember that many IFC.js functions are asynchronous, so we're going to use async and await.</IfcAlert>
...
<IfcAlert>This would be the same if we want to filter with other properties (e.g. the floor they belong to using the spatial structure tree, seen in the properties section). Just make sure you get the IDs of the elements you want to filter.</IfcAlert>
