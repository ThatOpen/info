...
This are not numbers, but **express IDs**! This means that you can use `getItemProperties()` again using that ID, and you'll get the properties of that **referenced** element.
...
Nonetheless, we usually recommend avoiding this in order to avoid runtime overheads.
