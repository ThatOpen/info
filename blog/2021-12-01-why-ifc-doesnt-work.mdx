...
The problem seems to be that **no large software company has a commercial interest in IFC**. In reality, the ultimate goal of all of them is to convince the user to use their entire ecosystem. The IFC is, in many cases, a partially flawed solution for relying on other tools that they may not have had time to implement IFC correctly or whose business is not in line with the interest of IFC.
