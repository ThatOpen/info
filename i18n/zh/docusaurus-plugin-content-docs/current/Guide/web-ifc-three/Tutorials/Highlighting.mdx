...
🎩
在几乎所有的 BIM 应用程序中，当用户将鼠标移到元素上或选择它们时，元素会被高亮显示。IFC.js 也不例外，在本教程中我们将看到如何实现这一点。
...
我们已经看到了如何通过
<a href="https://threejs.org/docs/#api/en/core/Raycaster">Raycaster</a>
来获得我们投射射线的对象的ID。现在我们有了这些ID，就可以使用<b>几何子集</b>
来突出显示这些元素。

<IfcCard icon="🧱" title="几何子集？">
  <b>几何子集</b>
  是模型中的任何一组项目。例如：所有的门、所有的底层元素或所有满足某个条件的元素。
</IfcCard>

### 进口依赖性

⚽
我们需要一个[material](https://threejs.org/docs/#api/en/materials/Material)来突出这些项目。你可以选择任何你喜欢的材料；在这个例子中，我们将使用一个[MeshLambertmaterial](https://threejs.org/docs/#api/en/materials/MeshLambertMaterial)，我们将从 Three 的核心库中导入。

```js
import { MeshLambertMaterial } from "three";
```

我们将使用 Raycaster，所以你还需要<a href="https://ifcjs.github.io/info/docs/Guide/web-ifc-three/Tutorials/Picking/#import-threejs-dependencies" target="_self">这些依赖项</a>，以及<a href="https://ifcjs.github.io/info/docs/Guide/web-ifc-three/Tutorials/Picking/#import-three-mesh-bvh-optional" target="_self">three-mesh-bvh</a>，如果我们想获得最佳性能。
...
- 如果你用材料 A 创建了一个墙的子集，然后试图用同样的材料 A 创建另一个墙的子集，第二个墙将被添加到第一个墙的子集中。

- 如果你用材料 A 创建了一个墙的子集，然后试图用材料 B 创建同一墙的另一个子集，你将创建两个独立的子集。
...
- 要创建子集的项目的 ID 必须以\*\*数组的形式给出（即使它是一个单一的 ID）。
...
🌚
在下一个例子中，我们将把透明材料应用于加载 IFC 模型的副本，当鼠标悬停在一个项目上时，用原始材料创建一个子集。为此，我们将使用与之前几乎相同的代码。
...
🎉🎉🎉
祝贺你! 你现在应该能够突出元素并从 BIM 模型中提取几何图形。干得好!

📃
然而，不要这么快就声称胜利。我们还没有谈到 BIM 中的 "我"，而这甚至比几何图形更重要。这就是下一课要讲的内容。
