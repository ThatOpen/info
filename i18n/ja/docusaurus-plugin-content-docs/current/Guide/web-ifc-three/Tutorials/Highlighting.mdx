...
## はじめに
  ...
  🎩
ほとんどのBIMアプリケーションでは、ユーザーが要素の上にマウスを移動させたり、選択したりすると、要素がハイライトされます。IFC.jsも例外ではありません。このチュートリアルでは、これを実現する方法を見ていきます。

<a href="https://ifcjs.github.io/info/docs/Guide/web-ifc-three/Tutorials/Picking/" target="_self">
    👩‍🏫
前回の</a>では、<a href="https://threejs.org/docs/#api/en/core/Raycaster">Raycaster</a>でレイをキャストしたオブジェクトのIDを取得する方法を見ました。これらのIDが得られたので、<b>幾何学的なサブセット</b>を使用してそれらの要素を強調することができます。

<IfcCard icon="🧱" title="幾何学的なサブセット？">
<b>幾何学的サブセット</b>は、モデル内のアイテムの任意のグループです。例えば、すべてのドア、すべての1階要素、または与えられた条件を満たすすべての要素などです。
</IfcCard>

### インポートの依存性

  🏀
アイテムを強調するためには、[material](https://threejs.org/docs/#api/en/materials/Material)が必要です。この例では、Threeのコアライブラリからインポートした[MeshLambertmaterial](https://threejs.org/docs/#api/en/materials/MeshLambertMaterial)を使用しています。
...
- 材料Aを使用した壁でサブセットを作成した後、同じ材料Aを使用した別の壁のサブセットを作成しようとすると、2番目の壁が最初の壁のサブセットに追加されます。

- 材料Aを使用して壁でサブセットを作成し、次に材料Bを使用して同じ壁の別のサブセットを作成しようとすると、2つの独立したサブセットが作成されます。
...
### シングルサブセット
 ...
 <p> 🐭
<a href="https://ifcjs.github.io/info/docs/Guide/web-ifc-three/api#createsubset" target="_self">createSubset</a>を使えば、ユーザーがホバーしたときにハイライト効果を出すことができます。</p>
...
- サブセットを作成するアイテムのIDは、**配列**として与えられなければなりません（単一のIDであっても）。
...
<IfcAlert>
  ...
  🌚
次の例では、読み込まれたIFCモデルに透明なマテリアルを適用し、マウスがアイテムの上に乗ったときに元のマテリアルでサブセットを作成します。これには先ほどとほぼ同じコードを使います。
...
🎉🎉🎉
おめでとうございます。これで、BIMモデルから要素をハイライトしたり、ジオメトリを抽出したりできるようになりました。お疲れ様でした。

  📃
しかし、すぐに勝利を主張してはいけません。BIMの "I "についてはまだ話していませんが、これはジオメトリよりもさらに重要なことです。それが次のレッスンの目的です。